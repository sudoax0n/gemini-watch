...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    ...
    dotenv_paths = [
        Path.home() / ".config" / "gemini-watch" / ".env",
        Path.cwd() / ".env",
    ]
...
def _post_whisper(endpoint: str, api_key: str, model: str, audio_path: Path) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "gemini-watch/1.0 (+gemini-cli; python-urllib)",
    }
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if attempt < MAX_ATTEMPTS - 1:
                print(
                    f"[gemini-watch] whisper HTTP {exc.code} — retrying in {delay:.1f}s "
            ...
            if attempt < MAX_ATTEMPTS - 1:
                ...
                print(
                    f"[gemini-watch] whisper network error ({type(exc).__name__}: {exc}) — "
...
def transcribe_video(
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            ...
            "in the environment or in ~/.config/gemini-watch/.env. "
            f"Run `python3 {setup_py}` to configure."
        )

    print(f"[gemini-watch] extracting audio for Whisper ({backend})…", file=sys.stderr)
    audio_path = extract_audio(video_path, audio_out)
    size_kb = audio_path.stat().st_size / 1024
    print(f"[gemini-watch] audio: {size_kb:.0f} kB — uploading to {backend} Whisper…", file=sys.stderr)
    ...
    print(f"[gemini-watch] transcribed {len(segments)} segments via {backend}", file=sys.stderr)
