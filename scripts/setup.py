#!/usr/bin/env python3
"""Setup / preflight for /gemini-watch.

Modes:
  setup.py --check      Silent preflight. Exit 0 if ready, 2/3/4 on failure.
  setup.py --json       Machine-readable status for Gemini to parse.
...
- Silent on success: --check exits 0 with no output when everything's ready so
  that /gemini-watch doesn't spam "setup is complete" on every turn.
- Idempotent: re-running the installer is safe — it never clobbers existing
  keys and only appends missing ones.
- SETUP_COMPLETE=true in ~/.config/gemini-watch/.env tells us the user has been
...
# Add Python Scripts directory to PATH on Windows if missing
if sys.platform == "win32":
    for base in (sys.executable, sys.exec_prefix, sys.base_exec_prefix):
        sdir = str(Path(base).parent / "Scripts" if Path(base).is_file() else Path(base) / "Scripts")
        if sdir not in os.environ.get("PATH", ""):
            os.environ["PATH"] = sdir + os.pathsep + os.environ.get("PATH", "")
    try:
        import site
        user_sdir = str(Path(site.getuserbase()) / "Scripts")
        if user_sdir not in os.environ.get("PATH", ""):
            os.environ["PATH"] = user_sdir + os.pathsep + os.environ.get("PATH", "")
    except Exception:
        pass


REQUIRED_BINARIES = ["ffmpeg", "ffprobe", "yt-dlp"]
CONFIG_DIR = Path.home() / ".config" / "gemini-watch"
CONFIG_FILE = CONFIG_DIR / ".env"
ENV_TEMPLATE = """# /gemini-watch API configuration
#
# Whisper transcription fallback — used only when yt-dlp cannot get captions
# (or when you point /gemini-watch at a local file with no subtitles).
#
...
#
# Leave both blank to disable Whisper — /gemini-watch will still work, but videos
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[gemini-watch] WARNING: {path} is readable by other users. "
...
def _scaffold_env() -> bool:
    """Create ~/.config/gemini-watch/.env with placeholders if missing."""
...
def cmd_install() -> int:
    ...
    if has_key:
        ...
        if installed_deps:
            print("[setup] installed dependencies; /gemini-watch is fully set up.")
        return 0
    ...
    print("")
    print("  Without a key, /gemini-watch still works but videos without captions come back frames-only.")
    return 3
